...
function gradeResults(data, expected) {
  ...
  if (t.kind && typeof t.kind === "object") {
    if ("Standard" in t.kind) out.gas_used = t.kind.Standard;
    else if ("Fuzz" in t.kind && t.kind.Fuzz?.median_gas != null) out.gas_used = t.kind.Fuzz.median_gas;
  ...
  // Success grading
  const testPassed = t.success === true;
