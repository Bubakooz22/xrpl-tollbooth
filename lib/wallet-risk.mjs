...
async function fetchSolOnchain(addr) {
  ...
  try {
    const r = await fetch('https://api.mainnet-beta.solana.com', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'getSignaturesForAddress', params: [addr, { limit: 10 }] })
    });
    const j = await r.json();
    if (j.error) return { codes, note: 'sol_' + j.error.message };
    const sigs = Array.isArray(j.result) ? j.result : [];
    if (sigs.length < 3) codes.push({ code: 'LOW_ACTIVITY', severity: 'low', source: 'onchain', evidence: 'signature_count=' + sigs.length });
    if (sigs.length) {
