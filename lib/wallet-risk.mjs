...
export function normalize(chain, addr) {
  if (chain === 'eth') return addr.toLowerCase();
  return addr.trim();
}

async function fetchEthOnchain(addr) {
  const key = process.env.ETHERSCAN_API_KEY || '';
  const codes = [];
  if (!key) return { codes, note: 'no_etherscan_key' };
  try {
    const nonceUrl = 'https://api.etherscan.io/api?module=proxy&action=eth_getTransactionCount&address=' + addr + '&tag=latest&apikey=' + key;
    const r1 = await fetch(nonceUrl);
    const j1 = await r1.json();
    const nonce = parseInt(j1.result || '0x0', 16);
    if (nonce < 5) codes.push({ code: 'LOW_ACTIVITY', severity: 'low', source: 'onchain', evidence: 'nonce=' + nonce });

    const txUrl = 'https://api.etherscan.io/api?module=account&action=txlist&address=' + addr + '&startblock=0&endblock=99999999&page=1&offset=100&sort=desc&apikey=' + key;
...
async function fetchSolOnchain(addr) {
  ...
  try {
    const r = await fetch('https://api.mainnet-beta.solana.com', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'getSignaturesForAddress', params: [addr, { limit: SOL_LIMIT }] })
    });
    const j = await r.json();
    if (j.error) return { codes, note: 'sol_' + j.error.message };
    const sigs = Array.isArray(j.result) ? j.result : [];
    // LOW_ACTIVITY only if we saw the whole history and it is tiny
    if (sigs.length < 3 && sigs.length < SOL_LIMIT) codes.push({ code: 'LOW_ACTIVITY', severity: 'low', source: 'onchain', evidence: 'signature_count=' + sigs.length });
    // NEW_ACCOUNT: only if we got the full history in one page (else the oldest sig is just paginated, not the actual first tx)
    if (sigs.length && sigs.length < SOL_LIMIT) {
      const oldest = sigs[sigs.length - 1];
      if (oldest.blockTime) {
        const now = Math.floor(Date.now() / 1000);
        if ((now - oldest.blockTime) < 7 * 86400) codes.push({ code: 'NEW_ACCOUNT', severity: 'low', source: 'onchain', evidence: 'first_tx ' + Math.floor((now - oldest.blockTime) / 3600) + 'h ago' });
      }
    }
    return { codes, note: null };
  } catch (e) {
...
export async function scoreWallet(rawAddr) {
  const chain = detectChain(rawAddr);
  ...
  let oc;
  if (chain === 'eth') { sources.push('onchain_eth'); oc = await fetchEthOnchain(norm); }
